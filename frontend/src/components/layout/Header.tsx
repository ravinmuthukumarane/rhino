...
export default function Header() {
  ...
  return (
    <header className="h-14 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-6 flex-shrink-0 gap-4">
      <h2 className="font-semibold text-gray-100 text-sm">{titles[location.pathname] ?? 'Energy Monitor'}</h2>
        ...
        {plants.length > 0 && (
          <div className="relative">
            <select value={selectedPlantId ?? ''}
              ...
              className="bg-gray-800 border border-gray-700 text-gray-200 text-xs rounded-lg px-3 py-1.5 pr-7 appearance-none focus:outline-none focus:ring-1 focus:ring-primary-500 cursor-pointer">
          ...
          {showAlerts && (
            <div className="absolute right-0 top-10 w-80 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl z-50">
              <div className="p-3 border-b border-gray-700 flex items-center justify-between">
                <span className="font-medium text-sm text-gray-200">Active Alerts ({unack})</span>
                <button onClick={() => setShowAlerts(false)} className="text-gray-500 hover:text-gray-200"><X className="w-4 h-4" /></button>
              </div>
              <div className="max-h-72 overflow-y-auto">
                {activeAlerts.length === 0
                  ? <p className="p-4 text-sm text-gray-500 text-center">No active alerts</p>
                  : activeAlerts.slice(0, 10).map((a) => (
                    <div key={a.id} className="p-3 border-b border-gray-800 flex items-start gap-2 last:border-0">
                      ...
                      <button onClick={() => dismissAlert(a.id)} className="text-gray-600 hover:text-gray-400 flex-shrink-0 mt-0.5">
