...
import { Factory, Plus, Edit2, Check, X, Zap, Droplets, Cpu, AlertCircle } from 'lucide-react';
...
// ── Inline edit row ──────────────────────────────────────────
function PlantRow({ plant, plants, onSave }: { plant?: Plant; plants: Plant[]; onSave: (data: object, id?: string) => void }) {
  const isNew = !plant;
  ...
  if (!open && !isNew) return (
    <tr className="border-b border-gray-800/50 hover:bg-gray-800/20">
      <td className="px-4 py-3 font-medium text-gray-200 text-sm">{plant!.name}</td>
      <td className="px-4 py-3 text-gray-400 text-sm">{plant!.location}</td>
      ...
      <td className="px-4 py-3"><button onClick={() => setOpen(true)} className="p-1.5 text-gray-500 hover:text-primary-400 hover:bg-primary-500/10 rounded transition-colors"><Edit2 className="w-3.5 h-3.5" /></button></td>
    </tr>
  );
    ...
    <tr className="border-b border-gray-700 bg-gray-800/20">
      ...
      <td className="px-4 py-2">
        <div className="flex gap-1">
          <button onClick={() => { onSave(f, plant?.id); if (!isNew) setOpen(false); }} className="p-1.5 text-green-400 hover:bg-green-500/10 rounded transition-colors"><Check className="w-3.5 h-3.5" /></button>
          {!isNew && <button onClick={() => setOpen(false)} className="p-1.5 text-gray-500 hover:text-red-400 rounded transition-colors"><X className="w-3.5 h-3.5" /></button>}
        </div>
      </td>
    </tr>
  );
...
function EnergyMeterRow({ meter, plants, onSave }: { meter?: EnergyMeter; plants: Plant[]; onSave: (data: object, id?: string) => void }) {
  const isNew = !meter;
  ...
  if (!open && !isNew) return (
    <tr className="border-b border-gray-800/50 hover:bg-gray-800/20">
      <td className="px-4 py-3 font-mono text-primary-400 text-sm">{meter!.meter_id}</td>
      <td className="px-4 py-3 text-gray-200 text-sm">{meter!.name}</td>
      <td className="px-4 py-3 text-gray-400 text-sm">{meter!.plant_name ?? '—'}</td>
  ...
  return (
    <tr className="border-b border-gray-700 bg-gray-800/20">
      ...
      <td className="px-4 py-2">
        <div className="flex gap-1">
          <button onClick={() => { onSave(f, meter?.id); if (!isNew) setOpen(false); }} className="p-1.5 text-green-400 hover:bg-green-500/10 rounded transition-colors"><Check className="w-3.5 h-3.5" /></button>
          {!isNew && <button onClick={() => setOpen(false)} className="p-1.5 text-gray-500 hover:text-red-400 rounded transition-colors"><X className="w-3.5 h-3.5" /></button>}
        </div>
      </td>
    </tr>
...
function FlowMeterRow({ meter, plants, onSave }: { meter?: FlowMeter; plants: Plant[]; onSave: (data: object, id?: string) => void }) {
  const isNew = !meter;
  ...
  if (!open && !isNew) return (
    <tr className="border-b border-gray-800/50 hover:bg-gray-800/20">
      <td className="px-4 py-3 font-mono text-orange-400 text-sm">{meter!.meter_id}</td>
      <td className="px-4 py-3 text-gray-200 text-sm">{meter!.name}</td>
      <td className="px-4 py-3 text-gray-400 text-sm">{meter!.plant_name ?? '—'}</td>
      <td className="px-4 py-3 text-gray-400 text-sm capitalize">{meter!.fluid_type}</td>
      <td className="px-4 py-3 text-gray-500 text-xs">{meter!.model ?? '—'}</td>
      <td className="px-4 py-3">{meter!.is_active ? <span className="badge-success">Active</span> : <span className="text-gray-500 text-xs">Inactive</span>}</td>
      <td className="px-4 py-3"><button onClick={() => setOpen(true)} className="p-1.5 text-gray-500 hover:text-orange-400 hover:bg-orange-500/10 rounded transition-colors"><Edit2 className="w-3.5 h-3.5" /></button></td>
    </tr>
  );
...
function GeneratorRow({ gen, plants, onSave }: { gen?: Generator; plants: Plant[]; onSave: (data: object, id?: string) => void }) {
  const isNew = !gen;
  ...
  if (!open && !isNew) return (
    <tr className="border-b border-gray-800/50 hover:bg-gray-800/20">
      <td className="px-4 py-3 font-mono text-yellow-400 text-sm">{gen!.generator_id}</td>
      <td className="px-4 py-3 text-gray-200 text-sm">{gen!.name}</td>
      <td className="px-4 py-3 text-gray-400 text-sm">{gen!.plant_name ?? '—'}</td>
      <td className="px-4 py-3 text-gray-300 text-sm">{gen!.capacity_kva ? `${gen!.capacity_kva} kVA` : '—'}</td>
      <td className="px-4 py-3 text-gray-400 text-sm capitalize">{gen!.fuel_type}</td>
      ...
      <td className="px-4 py-3"><button onClick={() => setOpen(true)} className="p-1.5 text-gray-500 hover:text-yellow-400 hover:bg-yellow-500/10 rounded transition-colors"><Edit2 className="w-3.5 h-3.5" /></button></td>
    </tr>
  );
  ...
  return (
    <tr className="border-b border-gray-700 bg-gray-800/20">
      ...
      <td className="px-4 py-2">
        <div className="flex gap-1">
          <button onClick={() => { onSave({ ...f, capacity_kva: f.capacity_kva ? parseFloat(f.capacity_kva) : null }, gen?.id); if (!isNew) setOpen(false); }} className="p-1.5 text-green-400 hover:bg-green-500/10 rounded transition-colors"><Check className="w-3.5 h-3.5" /></button>
          {!isNew && <button onClick={() => setOpen(false)} className="p-1.5 text-gray-500 hover:text-red-400 rounded transition-colors"><X className="w-3.5 h-3.5" /></button>}
        </div>
      </td>
    </tr>
...
export default function DeviceSettingsPage() {
  ...
  const tabs: { key: Tab; label: string; icon: any; count?: number; color: string }[] = [
    { key: 'plants', label: 'Plants', icon: Factory, count: plants.length, color: 'text-primary-400' },
    { key: 'energy', label: 'Energy Meters', icon: Zap, count: emeters.length, color: 'text-yellow-400' },
    { key: 'flow', label: 'Flow Meters', icon: Droplets, count: fmeters.length, color: 'text-orange-400' },
    { key: 'generators', label: 'Generators', icon: Cpu, count: generators.length, color: 'text-green-400' },
    { key: 'setpoints', label: 'Device Setpoints', icon: AlertCircle, color: 'text-blue-400' },
  ];

  return (
    <div className="space-y-5">
      <div className="card">
        <div className="flex items-center gap-2 mb-1"><Factory className="w-5 h-5 text-primary-400" /><h3 className="font-semibold text-gray-200">Device Settings</h3></div>
        <p className="text-sm text-gray-400">Assign energy meters, flow meters, and generators to plant locations. The simulator uses these for data generation.</p>
      </div>
        ...
        {tabs.map(({ key, label, icon: Icon, count, color }) => (
          <button key={key} onClick={() => { setTab(key); setAddNew(false); }}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-colors
              ${tab === key ? 'bg-gray-800 text-white shadow' : 'text-gray-400 hover:text-gray-200'}`}>
            <Icon className={`w-4 h-4 ${tab === key ? color : 'text-gray-500'}`} />
            <span className="hidden sm:inline">{label}</span>
            {count !== undefined && <span className={`text-xs ${tab === key ? 'text-gray-400' : 'text-gray-600'}`}>({count})</span>}
          </button>
        ))}
      </div>
      ...
      <div className="card overflow-hidden p-0">
        <div className="p-3 border-b border-gray-800 flex items-center justify-between">
          <h4 className="text-sm font-medium text-gray-200">{tabs.find((t) => t.key === tab)?.label}</h4>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            {tab === 'plants' && (<>
              <thead><tr className="border-b border-gray-800">{['Name','Location','Description','Devices','Status',''].map((h) => <th key={h} className="text-left px-4 py-3 text-xs text-gray-500 font-medium">{h}</th>)}</tr></thead>
              <tbody>
                {addNew && <PlantRow plants={plants} onSave={(d) => { plantMut.mutate({ data: d }); setAddNew(false); }} />}
                {plants.map((p) => <PlantRow key={p.id} plant={p} plants={plants} onSave={(d, id) => plantMut.mutate({ data: d, id })} />)}
            ...
            {tab === 'energy' && (<>
              <thead><tr className="border-b border-gray-800">{['Meter ID','Name','Plant','Model','Serial','Status',''].map((h) => <th key={h} className="text-left px-4 py-3 text-xs text-gray-500 font-medium">{h}</th>)}</tr></thead>
              <tbody>
                {addNew && <EnergyMeterRow plants={plants} onSave={(d) => { emMut.mutate({ data: d }); setAddNew(false); }} />}
                {emeters.map((m) => <EnergyMeterRow key={m.id} meter={m} plants={plants} onSave={(d, id) => emMut.mutate({ data: d, id })} />)}
                {emeters.length === 0 && !addNew && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No energy meters configured.</td></tr>}
              </tbody>
            </>)}

            {tab === 'flow' && (<>
              <thead><tr className="border-b border-gray-800">{['Meter ID','Name','Plant','Fluid Type','Model','Status',''].map((h) => <th key={h} className="text-left px-4 py-3 text-xs text-gray-500 font-medium">{h}</th>)}</tr></thead>
              <tbody>
                {addNew && <FlowMeterRow plants={plants} onSave={(d) => { fmMut.mutate({ data: d }); setAddNew(false); }} />}
                {fmeters.map((m) => <FlowMeterRow key={m.id} meter={m} plants={plants} onSave={(d, id) => fmMut.mutate({ data: d, id })} />)}
                {fmeters.length === 0 && !addNew && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No flow meters configured.</td></tr>}
              </tbody>
            </>)}

            {tab === 'generators' && (<>
              <thead><tr className="border-b border-gray-800">{['Generator ID','Name','Plant','Capacity','Fuel','Status',''].map((h) => <th key={h} className="text-left px-4 py-3 text-xs text-gray-500 font-medium">{h}</th>)}</tr></thead>
              <tbody>
                {addNew && <GeneratorRow plants={plants} onSave={(d) => { genMut.mutate({ data: d }); setAddNew(false); }} />}
                {generators.map((g) => <GeneratorRow key={g.id} gen={g} plants={plants} onSave={(d, id) => genMut.mutate({ data: d, id })} />)}
