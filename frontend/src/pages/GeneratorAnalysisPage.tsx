...
export default function GeneratorAnalysisPage() {
  ...
  return (
    <div className="space-y-5">
      <div className="card">
        <h3 className="font-semibold text-gray-200 flex items-center gap-2 mb-1">⚡ Generator Analysis</h3>
        <p className="text-sm text-gray-400">CEB vs Generator usage and power interruptions</p>
      </div>
        ...
        <>
          ...
          {data.monthly_summary && (
            <div className="grid grid-cols-4 gap-3">
              <div className="card">
                <p className="text-xs text-gray-400">CEB kWh</p>
                <p className="text-2xl font-bold text-blue-400">{parseFloat(data.monthly_summary.total_ceb_kwh).toFixed(0)}</p>
                <p className="text-xs text-gray-500">from grid</p>
              </div>
              <div className="card">
                <p className="text-xs text-gray-400">Generator kWh</p>
                <p className="text-2xl font-bold text-yellow-400">{parseFloat(data.monthly_summary.total_generator_kwh).toFixed(0)}</p>
                <p className="text-xs text-gray-500">from backup</p>
              </div>
              <div className="card">
                <p className="text-xs text-gray-400">Generator %</p>
                <p className="text-2xl font-bold text-orange-400">{data.monthly_summary.generator_percentage}%</p>
                <p className="text-xs text-gray-500">of total</p>
              </div>
              <div className="card">
                <p className="text-xs text-gray-400">Switchovers</p>
                <p className="text-2xl font-bold text-red-400">{data.monthly_summary.total_switchovers}</p>
          ...
          <div className="card">
            <h4 className="font-semibold text-gray-200 mb-4">Daily CEB vs Generator</h4>
          ...
          <div className="card">
            <h4 className="font-semibold text-gray-200 mb-4">Generator Usage %</h4>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.daily_breakdown?.map((d: any) => (
                    <tr key={d.date} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                      <td className="px-4 py-3 text-gray-200">{d.date}</td>
                      <td className="px-4 py-3">{parseFloat(d.ceb_kwh).toFixed(1)}</td>
                      <td className="px-4 py-3">{parseFloat(d.generator_kwh).toFixed(1)}</td>
                      <td className="px-4 py-3 font-bold text-blue-400">{parseFloat(d.total_kwh).toFixed(1)}</td>
                      <td className={`px-4 py-3 font-semibold ${parseFloat(d.generator_percentage) > 20 ? 'text-red-400' : 'text-green-400'}`}>
